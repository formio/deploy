...
{{- $fileStorageProvider := .Values.pdf.fileStorageProviderName -}}
{{- if not $fileStorageProvider }}
  {{- fail "A file storage provider (pdf.fileStorageProviderName) must be configured for PDF functionality" }}
{{- end }}
...
{{- if ne $fileStorageProvider "azure" }}
  ...
  {{- /* S3 Bucket - required for all S3-compatible providers */ -}}
...
{{- /* Handle injecting secret environment variables */ -}}
...
{{- else }}
{{- fail (printf "Required secret key '%s' not found" $req) }}
{{- end }}
...
{{- fail (printf "Required secret key '%s' was found but has an empty value" $req) }}
{{- end }}
{{- end }}
{{- end }}
{{- end }}
...
{{- end }}
{{- else }}
{{- /* For created secrets, only include keys that have values */ -}}
{{- range $key, $value := $newSecretValues }}
